...
class WebGraph():
    ...
    def power_method(self, v=None, x0=None, alpha=0.85, max_iterations=1000, epsilon=1e-6):
        ...
        with torch.no_grad():
            ...
            # Create "a" vector
            a = torch.sparse.sum(self.P,1)
            a = torch.mul(a, -1)
            a = torch.add(a.to_dense(), 1) # a vector has 1 in dangling nodes 
            # main loop
            k = 0
            while k < max_iterations:
                ...
                # Equation apply power method
                x0 = alpha* torch.sparse.mm(self.P.t(), x0).t() \
                     + (alpha*x0.t()*a + (1-alpha))*v.t()
                x0 = x0/torch.norm(x0)
                x0 = x0.t()

                if torch.norm(x0.t() - xk_minus1.t()) <= epsilon: # Exit condition
                    break
            x = x0.squeeze()
            return x
